    ...
    switch(pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            if(gUssdID == CALLINDEX_INVALID) //new USSD request
            {
                ...
                if(!ussd) {
                    KRIL_DEBUG(DBG_ERROR, "Unable to allocate ussd buf\n");  
                    pdata->handler_state = BCM_ErrorCAPI2Cmd;
                    return;
                }
...
void KRIL_CancelUSSDHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    {
        KRIL_DEBUG(DBG_ERROR,"CAPI2 response is NULL. handler_state:%d\n", pdata->handler_state);
        pdata->handler_state = BCM_ErrorCAPI2Cmd;
        return;
    }
    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ...
            else if (capi2_rsp->msgType == MSG_SS_CALL_FORWARD_STATUS_RSP)
            {
                ...
                {
                    int i;
                    Boolean ForwardStatusDisabled = TRUE;
                    ...
                    for(i = 0 ; i < rsp->class_size ; i++)
                    {
                        ...
                        /* to see if service is deactivated for all classes */
                        if( (rdata->call_forward_class_info_list[i].activated == 1) && (ForwardStatusDisabled == TRUE) )
                        {
                            ForwardStatusDisabled = FALSE;
                        }
                    ...
                    /* when service is deactivated for all classes, set the ss_class for all services to display properly */
                    if(ForwardStatusDisabled == TRUE)
                    {
                        rdata->class_size = 1;
...
void KRIL_SetCallForwardStatusHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch(pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            if ( (strlen(tdata->number)) && (mode == SS_MODE_ENABLE) )
            {
