...
void cam_WriteAutoIncrement1( UInt16 sub_addr, ... )
{
  ...
  //  char temp[8];
    ...
    {
        //HalcamTraceDbg("cam_WriteAutoIncrement1(): sCamI2cStatus ERROR: \r\n");
    }
}
...
void cam_WriteAutoIncrement2( UInt16 sub_addr, int *p )
{
    ...
    { 
      ...
      //  Log_DebugPrintf(LOGID_SYSINTERFACE_HAL_CAM, "cam_WriteAutoIncrement2(): i2c_sub_adr = 0x%x\r\n");
        ...
        {
            sCamI2cStatus |= result; 
            HalcamTraceErr("cam_WriteAutoIncrement2(): ERROR: ");
        }
    ...
    {
        HalcamTraceErr("cam_WriteAutoIncrement2(): sCamI2cStatus ERROR: ");
    }
}
...
static int CAMDRV_GetRegulator()//swsw_dual
{
	...
	if (!cam_regulator_i || IS_ERR(cam_regulator_i)) {
		HalcamTraceErr( "cori[cam_vddi]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	...
	if (!cam_regulator_a || IS_ERR(cam_regulator_a)) {
		HalcamTraceErr( "cori_[cam_vdda]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	cam_regulator_c = regulator_get(NULL, "cam_vddc");
	if (!cam_regulator_c|| IS_ERR(cam_regulator_c)) {
		HalcamTraceErr( "cori_[cam_vddc]No Regulator available");
		rc = -EFAULT;
		return rc;
	}
	return 1;
...
static int CAMDRV_SetRegulator()//swsw_dual
{
	...
	regulator_set_voltage(cam_regulator_c,1800000,1800000);	
	if (!cam_regulator_c){
...
static HAL_CAM_Result_en_t Init_cori_sensor(CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
    ...
    {
        HalcamTraceErr("pstSensorManager is NULL ");
    ...
    if( pstSensor == NULL )
    {
        HalcamTraceErr("pstSensor is NULL ");
        return HAL_CAM_ERROR_OTHERS;
    }
	...
	HalcamTraceDbg("write sr300pc20_init0");
	...
	if(iRet<0)
	{
      ...
      //reset settings	
       ...
       Drv_Mode = -1;
        ...
        HalcamTraceDbg("Init_cori_sensor end \n");
...
HAL_CAM_Result_en_t CAMDRV_Wakeup(CamSensorSelect_t sensor)
{
	...
	result = Init_cori_sensor(sensor);
	//Call cori_init() before
	HalcamTraceDbg("Init_torino_sensor :%d ",result);
	return result;
}
...
HAL_CAM_Result_en_t CAMDRV_SetImageQuality(UInt8 quality, CamSensorSelect_t sensor)
{
    ...
    HalcamTraceDbg("CAMDRV_SetImageQuality(): quality, Empty");
    return result;
}
...
#if 0 
	HalcamTraceDbg("write src200pc10_init0");
	// CamacqExtWriteI2cLists(sr200pc10_init0,1);   // wingi 
...
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	HalcamTraceDbg("CAMDRV_SetZoom(): Empty");
	return result;
}
					...
					CamSensorSelect_t sensor)
{
	...
	HalcamTraceDbg("CAMDRV_SetSceneMode() called, scene_mode = %d Drv_Scene=%d ",scene_mode,Drv_Scene);
	...
	switch(scene_mode) {
           case CamSceneMode_Auto:
	    {
		    ...
		    HalcamTraceDbg("gv_ForceSetSensor is TRUE ");			
               	}
	...
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetSceneMode(): Error[%d] ", sCamI2cStatus);
				     ...
				     CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
        ...
        HalcamTraceDbg("CAMDRV_SetWBMode() called, wb_mode = %d  Drv_WB= %d ",wb_mode,Drv_WB);

       if(((wb_mode==Drv_WB)||(Drv_Scene>CamSceneMode_Auto))&&(gv_ForceSetSensor==FALSE))	
       {
		HalcamTraceDbg("Do not set wb_mode,Drv_Scene=%d  ",Drv_Scene);
		Drv_WB= wb_mode;
		return HAL_CAM_SUCCESS;
	}	   	
    ...
    if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetWBMode(): Error[%d] ", sCamI2cStatus);
		result = sCamI2cStatus;
	}
    Drv_WB = wb_mode;
    return result;
				     ...
				     CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
        ...
        HalcamTraceDbg("CAMDRV_SetMeteringType() called,ae_mode = %d  Drv_ME=%d  ",type,Drv_ME);

       if(((type==Drv_ME)||(Drv_Scene>CamSceneMode_Auto))&&(gv_ForceSetSensor==FALSE))
       {
		HalcamTraceDbg("Do not set ae_mode Drv_Scene = %d ",Drv_Scene);
		return HAL_CAM_SUCCESS;
	}	   	
    ...
    switch( type )
    {
		...
		pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_AE_SPOT);
            break;
        default:
		HalcamTraceErr("not supported ae_mode ");
               
            break;
    }

    if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetMeteringType(): Error[%d] ", sCamI2cStatus);
		result = sCamI2cStatus;
	}
    Drv_ME = type;	
     ...
     if(gv_ForceSetSensor==TRUE)
     {
         gv_ForceSetSensor=FALSE;
          HalcamTraceDbg(" Reset  gv_ForceSetSensor =%d  ",gv_ForceSetSensor);
    }
...
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	HalcamTraceDbg("CAMDRV_SetAntiBanding() called Empty!!,effect = %d",effect);
...
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	HalcamTraceDbg("CAMDRV_SetFlashMode() called Empty!!,effect = %d",effect);
	
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetFlashMode(): Error[%d] ",sCamI2cStatus);
		result = sCamI2cStatus;
	}
    return result;
}
...
HAL_CAM_Result_en_t CAMDRV_SetFocusMode(CamFocusControlMode_t effect,
					CamSensorSelect_t sensor)
{
	HalcamTraceDbg("CAMDRV_SetFocusMode() called Empty!!, effect = %d ",effect);
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;

	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetFocusMode(): Error[%d] ",
			 sCamI2cStatus);
		result = sCamI2cStatus;
	}
	return result;
...
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	HalcamTraceDbg("CAMDRV_TurnOffAF() called Empty!! ");
	
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_TurnOnAF(): Error[%d] ",
			 sCamI2cStatus);
		result = sCamI2cStatus;
	}
	return result;
...
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	HalcamTraceDbg("CAMDRV_TurnOnAF() called Empty!! ");
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_TurnOnAF(): Error[%d] ",
			 sCamI2cStatus);
		result = sCamI2cStatus;
					    ...
					    CamSensorSelect_t sensor)
{
	...
	HalcamTraceDbg("CAMDRV_SetDigitalEffect() called,effect = %d Drv_Effect=%d ",effect,Drv_Effect);

       if(((effect==Drv_Effect)||(Drv_Scene>CamSceneMode_Auto))&&(gv_ForceSetSensor==FALSE))	
       {
		HalcamTraceDbg("Do not set effect Drv_Scene= %d ",Drv_Scene);
		return HAL_CAM_SUCCESS;
	}	   	
	...
	switch( effect )
	{
		...
		pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_EFFECT_SEPIA);
			break;
		default:
		HalcamTraceErr("not supported effect ");

			break;
	}

	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetDigitalEffect(): Error[%d] ", sCamI2cStatus);
		result = sCamI2cStatus;
	}
    Drv_Effect=effect;	
    return result;
...
HAL_CAM_Result_en_t CAMDRV_SetBrightness(CamBrightnessLevel_t brightness, CamSensorSelect_t sensor)
{
	...
	HalcamTraceDbg("CAMDRV_SetBrightness() called,brightness = %d Drv_Brightness=%d ",brightness,Drv_Brightness);

       if(((brightness==Drv_Brightness)||(Drv_Scene>CamSceneMode_Auto))&&(gv_ForceSetSensor==FALSE))		
       {
		HalcamTraceDbg("Do not set brightness Drv_Scene= %d ",Drv_Scene);
		return HAL_CAM_SUCCESS;
	}	   	
	...
	switch( brightness )
	{
             ...
             default:
                 HalcamTraceErr("not supported brightness ");
             break;
        }
msleep(40);//for resolving brightness changing error swsw
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetBrightness(): Error[%d] ", sCamI2cStatus);
		result = sCamI2cStatus;
	}
...
UInt8  CAMDRV_CheckEXP(UInt8 mode)
{
	...
	HalcamTraceDbg("CAMDRV_CheckEXP() called,mode = %d ",mode);
...
//           gv_checkEXPtime = ((float) Exptime/24000000.0f);
      ...
      HalcamTraceDbg("gv_checkEXPtime: %d, gv_checkEXPtime2 : %d ",gv_checkEXPtime, gv_checkEXPtime2);
#else
        if(mode == CAM_SHUTTER_SPEED)/*shutter speed */
        {
	      gv_checkEXPtime = 24000000/Exptime;
             HalcamTraceDbg("gv_checkEXPtime: %d ",gv_checkEXPtime);
		if(gv_checkEXPtime<=0)
		{
                  gv_checkEXPtime =1;
                  HalcamTraceDbg("gv_checkEXPtime 2: %d ",gv_checkEXPtime);
		}
        }
#endif
        ...
        {    //return Low_Lux; for prevent swsw
...
void  CAMDRV_CheckISO(void)
{
	...
	HalcamTraceDbg("CAMDRV_CheckISO() called ");
...
HAL_CAM_Result_en_t CAMDRV_SetJpegsize( CamImageSize_t stills_resolution,CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
        ...
        HalcamTraceDbg("CAMDRV_SetJpegsize() called, EMPTY!!");
...
HAL_CAM_Result_en_t CAMDRV_SetDTPmode(unsigned int testmode,CamSensorSelect_t sensor)
{
	...
	HalcamTraceDbg("CAMDRV_SetDTPmode() called,testmode = %d ",testmode);

       if(testmode==Drv_DTPmode)	
       {
		HalcamTraceDbg("Do not set DTPmode,Drv_DTPmode=%d  ",Drv_DTPmode);
		return HAL_CAM_SUCCESS;
	}	   	
	...
	if (sCamI2cStatus != HAL_CAM_SUCCESS) {
		HalcamTraceErr("CAMDRV_SetContrast(): Error[%d] ", sCamI2cStatus);
		result = sCamI2cStatus;
	}
...

{
    ...
    struct stCamacqSensorManager_t* pstSensorManager = NULL;
    struct stCamacqSensor_t* pstSensor = NULL;
    HalcamTraceDbg("%s(): , mode : %d ", __FUNCTION__, mode );
...
HAL_CAM_Result_en_t CAMDRV_GetESDValue( bool *esd_value,CamSensorSelect_t sensor)
{
	...
	struct stCamacqSensor_t* pstSensor = NULL;
      ...
      U8 Addr1=0x0b,Addr2=0x0c,Addr3=0x0d,Addr4=0xc1,Addr5=0xc2;
      U8	ReadValue = 0;
      //U32 sensor= 0;//Rear Camera
		
	//HalcamTraceDbg("CAMDRV_GetESDValue() called,= %d ");
	pstSensorManager = GetCamacqSensorManager();
	if( pstSensorManager == NULL )
	{
		HalcamTraceErr("pstSensorManager is NULL ");
	...
	//HalcamTraceDbg("%s() called !!!", __FUNCTION__);
	  ...
	  *esd_value = FALSE;
	  ...
	  //HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue1 = 0x%x ",ReadValue);
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[1]!!!!");
	       *esd_value = TRUE;
	       return result;
	  }

        CamacqExtReadI2c( pstSensor->m_pI2cClient, Addr2,1, &ReadValue,1);
	// HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue2 = 0x%x ",ReadValue);
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[2]!!!!");
	       *esd_value = TRUE;
	       return result;
	  }

        CamacqExtReadI2c( pstSensor->m_pI2cClient, Addr3,1, &ReadValue,1);
	 // HalcamTraceDbg("CAMDRV_GetESDValue() ReadValue3 = 0x%x ",ReadValue);
	  if(ReadValue!=0xaa)	
	  {
		HalcamTraceErr("ESD ERROR[3]!!!!");
	       *esd_value = TRUE;
	       return result;
	  }
...
//// init reglists valiable. ///////////////////////////////////////////////////////
        HalcamTraceDbg("CamacqExtWriteI2cLists ");
        ...
        while( !CAMACQ_MAIN_EXT_REG_IS_BTM_OF_DATA(rgucWriteRegs) )
        {
            if( CAMACQ_MAIN_EXT_REG_IS_DELAY(rgucWriteRegs) )
            {
                ...
                HalcamTraceDbg("CamacqExtWriteI2cLists : Case of Delay:(%d) ",usDealy);
            ...
            {
                {
                    //HalcamTraceDbg("CamacqExtWriteI2cLists : Case of Write:{0x%2x, 0x%2x} ", rgucWriteRegs[0], rgucWriteRegs[1]);
        ...
        HalcamTraceDbg("CamacqExtWriteI2cLists : END ");
    }
    return result;
}
#endif
