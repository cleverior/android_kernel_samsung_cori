...
static void determine_state_func(void)
{
	...
	else
	{
		...
		sync_use_mic = DISABLE;		
      set_irq_type(mic.headset_pd->hsirq, IRQF_TRIGGER_FALLING);
...
static void type_work_func(struct work_struct *work)
{
	...
	if(mic.headset_state)
	{
      ...
      if(mic.recheck_jack)
      {
         ...
         mic.recheck_jack = 0;
         adc = auxadc_access(2);
         printk("%s: recheck ADC=%d\n", __func__, adc);
         if(adc >= RECHECK_THRESHOLD)
         {
            mic.headset_state = 0;
...
static ssize_t recheck_Store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
   int recheck = 0;
   ret += sscanf(buf, "%d\n", &recheck);
...
static struct device_attribute recheck_Attrs = {
        .attr = { .name = "recheck", .mode = S_IWUG, .owner = THIS_MODULE },
        .store = recheck_Store,
};
...
static int hs_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&pdev->dev, &recheck_Attrs);
...
static int __init hs_probe(struct platform_device *pdev)
{
   ...
   result = device_create_file(&pdev->dev, &recheck_Attrs);
   if(result)
      device_remove_file(&pdev->dev, &recheck_Attrs);
