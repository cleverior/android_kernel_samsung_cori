	...
	)
{
    ...
    if(block)
    {
        ...
        while(1)
	    {
		    ...
		    if(len == 0) //FIFO empty sleep
			    return status;
//            if(arg_param) //snamy.jeong_120116 prevent error fixed
...
#if defined(CONFIG_TARGET_LOCALE_SWA_OPEN)
...
//	    else
//		    memset(&msgAudioCtrl.param, 0, sizeof(BRCM_AUDIO_Control_Params_un_t)); //snamy.jeong_120116 prevent error fixed
