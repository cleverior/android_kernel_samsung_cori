...
#if 1//defined(CONFIG_MFD_MAX8986)
...
#ifdef CONFIG_BACKLIGHT_KTD253B
...
extern int init_batt_lvl_interval;
...
static struct bcmsdhc_platform_data bcm21553_sdhc_data2 = {
	.base_clk = FREQ_MHZ(52),
	.flags = SDHC_DEVTYPE_EMMC | SDHC_CARD_ALWAYS_PRESENT | SDHC_DISABLE_PED_MODE,
	.cd_pullup_cfg = SDCD_PULLUP | SDCD_UPDOWN_ENABLE,
	.irq_cd = -1,
	.syscfg_interface = board_sysconfig,
	.cfg_card_detect = bcmsdhc_cfg_card_detect,
...
static struct bcm_keymap newKeymap[] = {
	...
	{BCM_KEY_ROW_0, BCM_KEY_COL_1, "key menu", KEY_MENU},
	...
	{BCM_KEY_ROW_1, BCM_KEY_COL_1, "key back", KEY_BACK},
	...
	{BCM_KEY_ROW_2, BCM_KEY_COL_1, "key home", KEY_HOME},
...
#if defined(CONFIG_TOUCHSCREEN_TMA340) || defined(CONFIG_TOUCHSCREEN_TAT200A) || defined(CONFIG_TOUCHSCREEN_TMA140)
static struct bcm_keymap newKeymap[] = {
	{BCM_KEY_ROW_0, BCM_KEY_COL_0, "Volume Up", KEY_VOLUMEUP},
...
#ifdef CONFIG_BACKLIGHT_KTD253B
struct platform_device bcm_ktd253b_backlight_devices = {
	.name = "sec-backlight", 
	.id = 0,
};
static struct platform_ktd253b_backlight_data bcm_ktd253b_backlight_data = {
	.max_brightness = 255,
	.dft_brightness = 143,
	.ctrl_pin = 17,
};
...
/* DLG start*/
#if defined(CONFIG_MFD_D2041)
static struct linear_power_pdata power_pdata = {
	.usb_charging_cc = MAX8986_CHARGING_CURR_450MA,
	.wac_charging_cc = MAX8986_CHARGING_CURR_450MA,
	.eoc_current = MAX8986_EOC_100MA,
...
//	static u16 bat_adc[] = {0x2BE, 0x2D3, 0x2E8, 0x2FC, 0x311, 0x32C, 0x364};
	...
	static u32 TempAdcTable[] = {     775,721,664,602,541,479,420,366,315,270,230,196,166,141,119,100,93,88,}; 
#else
	static u32 TempAdcTable[] = {     769,685,621,579,502,442,387,336,293,264,212,180,155,132,114,98,}; 
#endif
	static u32 TempDegreeTable[] ={-10,-5,     0,    5,  10,  15,  20, 25,  30,  35,  40, 45,  50,  55,  60, 65,};
...
//		BattEmptyThresh = SYSPARM_GetBattEmptyThresh();
	...
	/* init batt params */
	power_pdata.batt_adc_tbl_t.num_entries = ARRAY_SIZE(bat_adc);
	power_pdata.batt_adc_tbl_t.bat_adc = bat_adc;
	power_pdata.batt_adc_tbl_t.bat_vol = bat_vol;
	break;
	 ...
	 * the usb stack is still not up.
	 */
	if (usb_charger_detected == false && ((param == PMU_MUIC_CHGTYP_USB) || (param == PMU_MUIC_CHGTYP_DOWNSTREAM_PORT))) {
		usb_charger_detected = true;
		pmu_usb_start();
	}
	...
	case PMU_EVENT_CHARGER_REMOVE:
	pr_info("%s: PMU_EVENT_CHARGER_REMOVE (%d)\n", __func__, param);
	if (usb_charger_detected == true && ((param == PMU_MUIC_CHGTYP_USB) || (param == PMU_MUIC_CHGTYP_DOWNSTREAM_PORT))) {
		usb_charger_detected = false;
		pmu_usb_stop();
	}
...
static void Linear_Charger_GpioInit_Setting(void)
{

	pr_info("%s: Charger: init hardware\n", __func__);
	bcm_gpio_pull_up(MUSB_SDA,true);
	bcm_gpio_pull_up(MUSB_SCL,true);
	bcm_gpio_pull_up(MUSB_IRQ_GPIO,true);	
	gpio_direction_input(CHG_STATE);
	bcm_gpio_pull_up_down_enable(CHG_STATE, false);
	gpio_direction_output(CHG_EN,1);
}

/* D2041 regulator mapping */
#define LDO_UNDEFINED	(-1)
struct regulator_consumer_supply d2041_consumers[D2041_NUMBER_OF_REGULATORS] = {
	[D2041_LDO_2] = REGULATOR_SUPPLY("aldo1_3v0", NULL),
	[D2041_LDO_11] = REGULATOR_SUPPLY("sim_vcc", NULL),
	[D2041_LDO_13] = REGULATOR_SUPPLY("sd_a_vdd", NULL),
	[D2041_LDO_14] = REGULATOR_SUPPLY("touch_vcc", NULL),
	[D2041_LDO_15] = REGULATOR_SUPPLY("vib_vcc", NULL),
	[D2041_LDO_16] = REGULATOR_SUPPLY("cam_vddc", NULL),
	[D2041_LDO_19] = REGULATOR_SUPPLY("cam_vddi", NULL),
	[D2041_LDO_20] = REGULATOR_SUPPLY("cam_vdda", NULL),
	[D2041_BUCK_1] = REGULATOR_SUPPLY("csr_nm2", NULL),
};



struct d2041_audio_platform_data audio_pdata = {
	.ina_def_mode =	D2041_IM_FULLY_DIFFERENTIAL,
	.inb_def_mode = D2041_IM_TWO_SINGLE_ENDED,
	.ina_def_preampgain = D2041_PREAMP_GAIN_NEG_6DB, //D2041_PREAMP_GAIN_18DB,
	.inb_def_preampgain = D2041_PREAMP_GAIN_NEG_6DB, //D2041_PREAMP_GAIN_18DB,

	.lhs_def_mixer_in = D2041_MSEL_B1,
	.rhs_def_mixer_in = D2041_MSEL_B2,
	.ihf_def_mixer_in = D2041_MSEL_A2,

	.hs_input_path = D2041_INPUTB,
	.ihf_input_path = D2041_INPUTA,
};
...
struct d2041_platform_data d2041_pdata = {	
	.i2c_pdata = {.i2c_spd = I2C_SPD_400K,},
	.audio_pdata = &audio_pdata,
	.regulator_data = d2041_consumers,
	.power = &power_pdata,
	.pmu_event_cb = pmu_event_callback,
	.regl_map = {
	    D2041_MCTL_MODE_INIT(D2041_BUCK_1, 0xDE/*0xCD*/, D2041_REGULATOR_LPM_IN_DSM),	// MAX8986 -> CSR_LPM
        D2041_MCTL_MODE_INIT(D2041_BUCK_2, 0x56/*0x01*/, D2041_REGULATOR_MAX),			// MAX8986 -> IOSR
        D2041_MCTL_MODE_INIT(D2041_BUCK_3, 0x56/*0x01*/, D2041_REGULATOR_MAX),			// MAX8986 -> TSR
        D2041_MCTL_MODE_INIT(D2041_BUCK_4, 0x44/*0x00*/, D2041_REGULATOR_MAX),
        D2041_MCTL_MODE_INIT(D2041_LDO_1,  0x66, D2041_REGULATOR_MAX),						// LVLDO_1.2V
        D2041_MCTL_MODE_INIT(D2041_LDO_2,  0x44, D2041_REGULATOR_OFF_IN_DSM),	// 0x22		// ALDO_3V0
        D2041_MCTL_MODE_INIT(D2041_LDO_3,  0x66, D2041_REGULATOR_MAX),			// 0x11
        D2041_MCTL_MODE_INIT(D2041_LDO_4,  0x66, D2041_REGULATOR_MAX),			// 0x11
        D2041_MCTL_MODE_INIT(D2041_LDO_5,  0x66, D2041_REGULATOR_MAX),			// 0x11
        D2041_MCTL_MODE_INIT(D2041_LDO_6,  0x56, D2041_REGULATOR_MAX),			// 0x01
        D2041_MCTL_MODE_INIT(D2041_LDO_7,  0x66, D2041_REGULATOR_MAX),			// 0x11		// VLCD
        D2041_MCTL_MODE_INIT(D2041_LDO_8,  0x66, D2041_REGULATOR_MAX),			// 0x11
        D2041_MCTL_MODE_INIT(D2041_LDO_9,  /*0x55*/0xAA, D2041_REGULATOR_MAX),	// 0x55
        D2041_MCTL_MODE_INIT(D2041_LDO_10, 0x00, D2041_REGULATOR_MAX),						// spare
        D2041_MCTL_MODE_INIT(D2041_LDO_11, 0x22/*0x66*/, D2041_REGULATOR_LPM_IN_DSM),	// 0x11		// VSIM_3.0V
        D2041_MCTL_MODE_INIT(D2041_LDO_12, 0x66, D2041_REGULATOR_LPM_IN_DSM),	// 0x11		// VDD_MEM_3V3
        D2041_MCTL_MODE_INIT(D2041_LDO_13, 0x66, D2041_REGULATOR_OFF_IN_DSM),	// 0x11		// DLDO1_3.0V
        D2041_MCTL_MODE_INIT(D2041_LDO_14, 0x44, D2041_REGULATOR_OFF_IN_DSM),	// 0x22		// TOUCH
        D2041_MCTL_MODE_INIT(D2041_LDO_15, 0x00, D2041_REGULATOR_MAX),			// 0xAA		// VIB
        D2041_MCTL_MODE_INIT(D2041_LDO_16, 0x00, D2041_REGULATOR_OFF_IN_DSM),	// 0xAA		// cam_vddc
        D2041_MCTL_MODE_INIT(D2041_LDO_17, 0x44, D2041_REGULATOR_MAX),			// 0x22		// VBBD
        D2041_MCTL_MODE_INIT(D2041_LDO_18, 0x00, D2041_REGULATOR_MAX),						// spare	
        D2041_MCTL_MODE_INIT(D2041_LDO_19, 0x00, D2041_REGULATOR_OFF_IN_DSM),	// 0xAA		// cam_vddi
        D2041_MCTL_MODE_INIT(D2041_LDO_20, 0x00, D2041_REGULATOR_OFF_IN_DSM),	// 0xAA		// cam_vdda
		D2041_MCTL_MODE_INIT(D2041_LDO_AUD,0x00, D2041_REGULATOR_OFF_IN_DSM),
	},
};
...
#endif
#if defined(CONFIG_TOUCHSCREEN_TAT200A)
	{
				I2C_BOARD_INFO("synaptics-rmi-ts", 0x20),
				.irq = GPIO_TO_IRQ(TSP_INT),
	},
...
/* DLG start */
{
	I2C_BOARD_INFO("d2041", 0x49),
	.platform_data = (void *)&d2041_pdata,
	.irq = GPIO_TO_IRQ(34), //Check this one
}
#endif /*CONFIG_MFD_MAX8986*/
};
		...
		if (op == SYSCFG_INIT) {
			...
			writel((readl(ADDR_SYSCFG_IOCR0) |
				SYSCFG_IOCR0_FLASH_SD2_MUX),
			       ADDR_SYSCFG_IOCR0);
			/* Offset for IOCR2 = 0x0c - no pullup of DAT[0:3]/CMD */
			writel(readl(ADDR_SYSCFG_IOCR2) &
			       ~(SYSCFG_IOCR2_SD2CMD_PULL_CTRL(SD_PULL_UP | SD_PULL_DOWN) |
				 SYSCFG_IOCR2_SD2DATL_PULL_CTRL(SD_PULL_UP | SD_PULL_DOWN)),
			       ADDR_SYSCFG_IOCR2);
			/*no pullup of DAT[7:4]/CMD */
			writel(readl(ADDR_SYSCFG_IOCR6) & 
					~(SYSCFG_IOCR6_SD2DATH_PULL_CTRL(SD_PULL_UP | SD_PULL_DOWN)),
					ADDR_SYSCFG_IOCR6);
			...
			/* Offset for IOCR6 */
			writel(readl(ADDR_SYSCFG_IOCR6)
			       | SYSCFG_IOCR6_SD2DATH_PULL_CTRL(SD_PULL_UP),
			       ADDR_SYSCFG_IOCR6);
		} else if (op == SYSCFG_DISABLE) {
			/* Offset for IOCR2 = 0x0c */
			writel(readl(ADDR_SYSCFG_IOCR2) &
...
#define EXT_CTRL_REQ2	4
			...
			Linear_Charger_GpioInit_Setting();
		}
		break;
...
#endif
#ifdef CONFIG_BACKLIGHT_KTD253B
	bcm_ktd253b_backlight_devices.dev.platform_data = &bcm_ktd253b_backlight_data;
#endif
#ifdef CONFIG_KEYBOARD_BCM
...
#define GPIPUD_PULL_DOWN(GPIO) (~(1<<(GPIO%32)))
				...
				writel(readl(ADDR_GPIO_GPIPUD1)&GPIPUD_PULL_DOWN(42),ADDR_GPIO_GPIPUD1);
				...
				/*Set as pull down*/
				//writel(readl(ADDR_GPIO_GPIPUD1)|GPIPEN_PULL_EN(35),ADDR_GPIO_GPIPUD1);
